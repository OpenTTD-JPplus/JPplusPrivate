...
function setHeadGfx(dataArr) {
  const { trainName, totalVariations, introYear, modernizationYear, spriteArray, switchCounter } =
    dataArr || null;
  //error handling
  if (introYear && modernizationYear)
    throw new Error("Intro and Modernization params can't be used at the same block!");
  ...
  if (modernizationYear) {
    str += createSwitch(
      "SELF",
      `sw_${trainName}_${direction}${
        totalVariations == 1 || totalVariations === switchCounter
          ? "_main"
          : `${switchCounter ? `_${switchCounter}` : ""}`
      }`,
