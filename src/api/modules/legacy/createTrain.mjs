import { setCapacity } from "../setCapacity.mjs";
...
import { setPower } from "../setPower.mjs";
...
export function createTrain(data, path) {
  ...
  let res =
    `${
      customPuchaseSpriteLogic
        ? customPuchaseSpriteLogic
        : setPurchaseSprite(trainName, path, `logo_${data.operator.toLowerCase()}`, [0, 50])
    }` +
    "\n" +
