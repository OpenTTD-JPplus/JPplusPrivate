export function setPurchaseSprite(trainName, path, logo, offsets, length = 8, isSingle = 0, is8bpp = 0) {
  ...
  return (
    `spriteset(gfx_${trainName}_purchase_c1, ${is8bpp ? "" : "ZOOM_LEVEL_NORMAL, BIT_DEPTH_32BPP,"}"${fullPath}"){ tmpl_purchase_1c_dual(0, ${
      offsets[0]
    }) }\n` +
    `spriteset(gfx_${trainName}_purchase_c2, ${is8bpp ? "" : "ZOOM_LEVEL_NORMAL, BIT_DEPTH_32BPP,"}"${fullPath}"){ tmpl_purchase_2c_dual(0, ${
      offsets[0]
    }) }\n` +
...
export function setSprites(trainName, path, spriteSets, is8bpp = false) {
  ...
  spriteSets.forEach((spriteGroup, i) => {
    ...
    spriteGroup.forEach((sprite) => {
      ...
      }"${path}/${trainName}/${trainName}${spriteSets.length > 1 ? `_${i + 1}` : ""}.png"){ ${sprite[1]} }\n`;
    });
  });
  return str;
}
