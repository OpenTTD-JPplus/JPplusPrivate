...
function setPowerVariant(data) {
  ...
  str += createSwitch(
    "SELF",
    `sw_${trainName}_car_power${
      totalVariations == 1 ? "_main" : `${switchCounter ? `_${switchCounter}` : ""}`
    }`,
    `position_in_vehid_chain%${blockLength}+1`,
    positionData
  );
  // }

  let headPowerString = "0";
  ...
  str += createSwitch(
    "SELF",
    `sw_${trainName}_power${
      totalVariations == 1 ? "_main" : `${switchCounter ? `_${switchCounter}` : ""}`
    }`,
    `vehicle_type_id == ${trainName}`,
    {
      1: headPowerString,
      default: `sw_${trainName}_car_power${
        totalVariations == 1 ? "_main" : `${switchCounter ? `_${switchCounter}` : ""}`
      }`,
    }
  );
...
export function setPowerMain({ trainName, data }) {
  ...
  if (totalVariations > 1 && !data[0].introYear)
    throw new Error("Can't use 2 power variants without year data");
