...
item(FEAT_TRAINS, ${trainName}) {
  property {
    ...
    model_life: ${scrapYear == 0 ? "VEHICLE_NEVER_EXPIRES" : scrapYear - introductionDate.split(",")[0] + 5};
    ...
    power: sw_${trainName}_power_head_main()*4*1342/1000;
    purchase_power:${power}*4*1342/1000;
  }
  livery_override(mu_car){
		cargo_subtype_text:${liveries.length > 1 ? `cargo_subtype_text:sw_${trainName}_lv_desc_main;` : ""};
    loading_speed:${doors.length > 1 ? doors : `sw_${trainName}_loading_speed_main`};
    cargo_capacity: sw_${trainName}_capacity_wag_main()*param_capacity_mod/3${capacity[0].headCapacity < 50 ? "*boost_rapid_mod/2" : ""};
    power: sw_${trainName}_power_cars_main()*4*1342/1000;
    default:sw_${trainName}_lv;
    weight: ${weight[1]};
  }
}`;
